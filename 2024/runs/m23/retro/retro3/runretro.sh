...
#for j in `seq 1 5`;
  ...
  for i in `seq $2 $1`;
    do
      awk -v rrr=$i 'NR==157{print rrr} NR!=157 {print $0}' t_start.ctl >control.dat
      ./pm -nox -iprint 500 
	    /bin/cp -f control.dat r_control_$i.dat
      /bin/cp -f pm.std r_$i.std
	    /bin/cp -f pm.rep r_$i.rep
	    /bin/cp -f F40_t.rep r_F40_$i.rep
	    /bin/cp -f selgrid.rep r_selgrid_$i.rep
    done    
		#mv retro retro$j
		#mkdir retro
  #done    
