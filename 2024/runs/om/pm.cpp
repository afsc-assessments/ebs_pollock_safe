...
void model_parameters::Evaluate_Objective_Function(void)
{
  ...
  // if (active(log_Rzero) & !do_srrdevs) Priors(4) = 12.5*square( log_Rzero - 10.23 ); 
...
void model_parameters::SimulateData1(void)
{
 ...
 // 2010 survey timing N
  ...
  for (int isim=1;isim<=nsims;isim++)
  {
    ...
    simwrite( obs_avo);
    simdat << avo_tmp*wt_avo(n_avo)*q_avo   <<endl;
    simwrite( obs_avo_std);
    simdat << 0.25*avo_tmp*wt_avo(n_avo)*q_avo   <<endl;
    simwrite( wt_avo);
  ...
  // if(Sim_status==1) 
    ...
    simwrite( obs_bts_data); 
		simdat<<" "<< bts_tmp * wt_bts(n_bts)*q_bts<<endl;
    // 20% CV for new data
    simwrite(std_ob_bts_data) ; 
		simdat<<" "<< eac_bts_fut*wt_bts(n_bts)*.2*q_bts <<endl;
    simwrite(wt_bts ); 
		simdat << wt_bts(n_bts)             <<endl;
    simwrite(std_ot_bts); 
		simdat << " "<< sum(eac_bts_fut)*.15 <<endl;
    ...
    simwrite(std_ot_ats); 
		if (new_ats)
			simdat <<" "<< sum(ats_tmp)*.2<<endl;
    simwrite( obs_ats_data); 
		if (new_ats)
	  	simdat<<" "<<ats_tmp * wt_ats(n_ats)*q_ats<<endl;
