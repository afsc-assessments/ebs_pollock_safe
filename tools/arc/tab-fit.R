#' Tabulate fits from models using specific metrics.
#'
#' This function takes a list of model outputs and compiles a dataframe with specific fit metrics.
#'
...
#'
#' @return Returns a dataframe with fit metrics.
#' @export
tab_fit1 <- function(M, mod_scen=NULL){
  ...
  for (ii in mod_scen) {
    ...
    metrics <- c("rmse_bts", "rmse_ats", "rmse_avo", "rmse_cpue", "sdnr_bts", "sdnr_ats", "sdnr_avo",
                 "EffN_fsh", "EffN_bts", "EffN_ats", "surv_like", "age_like")
    v <- c()
    for (metric in metrics){
      if(metric == "EffN_fsh"){
        tmp <- round(mean(x[[metric]][28:dim(x[[metric]])[1], 2]), 2)
      } elseif(metric %in% c("EffN_bts", "EffN_ats")) {
