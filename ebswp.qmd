...
\begin{tabular}{lrr|rr}
  ...
  $F_{OFL}$    & `r M$it[1,8]` & `r M$it[2,8]` & `r  M$Tier3_FOFL1s`  & `r  M$Tier3_FOFL2s `  \\                  
  $maxF_{ABC}$ & `r M$it[1,9]` & `r M$it[2,9]` & `r  M$Tier3_FABC1s `  & `r  M$Tier3_FABC2s `  \\                  
  $F_{ABC}$    & `r M$it[1,10]`& `r M$it[2,10]`& `r  (M$Tier3_FABC1s)` & `r  (M$Tier3_FABC2s)` \\                  
  $OFL$        & `r M$it$e1[1]` t & `r M$it$e2[1]` t & `r  M$Tier3_OFL1s`  t & `r  M$OFL2s`  t \\          
...
[xxGiven the same estimated aggregate fishing effort as in 2024 and given]{bg-colour="#FFFF00" }
the estimated stock trend, the constant-F scenario would yield about 1.3
million t. To obtain 2025 catches on the order of 1.45 million t, given
the base model estimates, would require about 18% more effort than what
was estimated from 2022.xx
