...
disseminated by the National Marine Fisheries Service and should not be
...
for index standardization (and extended into the NBS). 
...
biomass  estimated from 1987. The values for the recent years appear to have stabilzed
well above $B_{MSY}$. This continues to be due to the strength of the 2018
year-class of the pollock population.
...
at the AFSC, we presented in @ianelli2023a the work noted that the
...
(\cref{tbl:catch}). A fishing moratorium for this area was enacted in
...
in 2009 to nearly 1.5 million t during 2002--2006 (\cref{tbl:catch}).
...
salmon bycatch rate (@fig-fsh_psc_cpue). The nominal catch rate of
sablefish in the pollock fishery continue to be above historical
averages (@fig-fsh_psc_cpue) while for herring, the rate was low
compared to 2020.
...
The 2023 A-season fishery spatial pattern had a relatively more catch
around the Pribilof Islands compared to 2021 (@fig-catch_distn_a). The
...
levels for all fleet sectors (middle panel, @fig-fsh_cpue). Beginning in
...
(@fig-fsh_cpue). The number of hours the fleet required to catch the
same tonnage of pollock was also improved relative to 2020. In the
B-season catches in the northwestern area increased relative to the
...
results indicated that in the A-season dispersion increased slightly but
for the B-season in 2023, the fleet appeared to be less dispersed than
...
pattern was smaller than expected. Compiling the data by week we show
that the fish size was consistent with the pattern of fish being
...
By 2021 and 2022, the predominance of 3- and 4-year olds in the catch   
confirms the abundant year-class from 2018. We note that the center of
...
the Russian border and the NBS region, and
treatment of the extent stock shifts between regions continues (e.g.,
@oleary2021).

The 2024 survey estimate is the highest estimate since 2019, 90% higher than
the 2023 estimate and 36% higher than the time-series mean. The 2024 pollock 
density by station increased relative to 2023, with the largest increases 
along the shelf (@fig-bts_3d). The VAST model provides density-weighted 
...
but shifted south in 2022 and is slightly further south in 2024, after a 
northward shift in 2023. The stock center of gravity also moved east from 2010 
to about 2017, then shifted west. The 2024 estimate is further west than in 
2023 (@fig-cog).
...
abundance of 5-year old pollock (the 2018 year-class) decreased from 2022 to 
...
estimates increased to over 4 million t (\cref{tbl:atsbiom}). The number
of trawl hauls, lengths, and ages sampled from the AT survey are
presented in \cref{tbl:atsn}. These surveys have also provided insight
...
data throughout the typical ATS geographic footprint. The new methods
...
1)  The 2024 AVO index of midwater pollock abundance on the EBS shelf
    ...
    lowest estimate since 2014. This compares with the 2024 AFSC
    biennial acoustic-trawl survey (ATS) conducted using NOAA Ship Oscar
    Dyson decreased 25% from 2022.
...
4)  The strongest pollock backscatter during the AVO and AT surveys was
    ...
    center of gravity estimate for the 2024 AVO survey was similar to
...
for the same seven ATS-BTS years from the original index). The new and
rescaled index trend dropped 15% relative to 2022 but still is 13% above
the long-term mean (@fig-avo, \cref{tbl:avo}; note that the relative
error is based on a variance estimation from (@petitgas1993), while the
...
[calculations so far incomplete...]{bg-colour="#FFFF00" }
...
through the population (@fig-mod_ats_age).
...
We added code for producing posterior predictive distributions (e.g.,
...
new data from 2024 had very minor changes and impact on the spawning
biomass estimates (@fig-mod_data; top panel). The largest effect on all
the changes arose from the revision to the mean body weight-at-age used
for the spawning biomass calculations (@fig-mod_data; bottom panel).
This was shown in @ianelli2023. Nonetheless, diagnostics of all the
changes relative to model fits are given in \cref{tbl:modfits} and a
comparison of management quantities for the final base model is given in
\cref{tbl:mgtquants}).
...
(\cref{tbl:biom3plus}). Historically, biomass levels increased from 1979
...
Estimated numbers-at-age are presented in (\cref{tbl:estn}) and
...
to be XXX0.262XXX for EBS pollock (assuming this year's estimated 2023
selectivity and weight-at-age).
...
the stock trend would be stable and yield about 1.3 million t (Table 34).
...
The 2024 fishery
xxxseemed to have generally smaller-than-expected pollock in the fishery
...
low (indicating relatively good fishing; @fig-fleet_dispersal).
...
(\cref{tbl:dctbl}) and the fact that the biological basis for the
...
\cref{tbl:dectabledesc}). Model 23 (the "base") results for these
indicators are provided in Table 34.
<!-- \cref{tbl:dectbl}.  -->
...
xxxSince the pollock fishery is primarily pelagic in nature, the bycatch
of non- target species is small relative to the magnitude of the fishery
(\cref{tbl:nontargbycatch}). Jellyfish represent the largest component
...
2023 & xxxxx9  & 46,843  & 43,299  & 47,450  & 16,811  & 18,006  & 229,868 \\
\hline
\end{tabular}
}
\end{table}
...
2024    &   11 Jun- 19 Jul      &   xxx,942 &   0.488   &   0.583   &   1.799   &   2.871   &   0.056   &   26\%    \\
\hline
\end{tabular}
}
\end{table}
...
tab <- xtable(df,
  caption = "\\label{tbl:modfits}Goodness-of-fit measures to primary data for different assessment model configurations.
...
tab <- xtable(t3, caption = cap, label = paste0("tbl:", tablab[32]), digits = 0, auto = TRUE, align = rep("r", (length(t3[1, ]) + 1)))
...
tab <- xtable(tmp,
  digits = 2, auto = TRUE, caption = "\\label{tbl:estn}Estimated billions of EBS pollock at age (columns 2--11) from the current assessment model.",
  label = "tbl:estn"
)
...
colnames(tmp) <- c("Year", 1:9, "10+")
tab <- xtable(tmp,
  digits = 2, auto = TRUE, caption = "\\label{tbl:estcatage}Estimated millions of EBS pollock caught at age (columns 2--11) from the current assessment model.",
  label = "tbl:estcatage"
)
print(tab, tabular.environment = "longtable", caption.placement = "top", 
 include.rownames = FALSE, sanitize.text.function = function(x) { x }, 
...
tab <- xtable(m, caption = cap, label = "tbl:dectbl", digits = 0, auto = TRUE, align = rep("r", (length(m[1, ]) + 1)))
...
printfig("fsh_wtage_strata_yr.png", 22)
```
